...
from .modeling_vts_qwen import VTS_Qwen2_5_VL, VTS_Qwen2_5_VLConfig
...
def build_model(
    ...
    """
    processor = AutoProcessor.from_pretrained(base_model_path, do_resize=False)
    config = AutoConfig.from_pretrained(base_model_path)

    if vts_ratio:
        if hasattr(config, "VTS_token_ratio"):
            print(f"Original VTS_token_ratio: {config.VTS_token_ratio}")
            config.VTS_token_ratio = vts_ratio
            print(f"Updated VTS_token_ratio: {config.VTS_token_ratio}")
        else:
            print("Warning: config has no VTS_token_ratio attribute.")
