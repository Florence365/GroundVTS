...
def convert_tacos_to_llamafactory_json(jsonl_path, video_dir, save_path,
                                       prompt_templates=None,
                                       seed: int = 42):
    """
    Convert TACoS JSONL data to LLaMA-Factory format JSON.
    Format is similar to QVHighlights JSONL.

    Args:
        jsonl_path: Path to the original TACoS JSONL file.
    ...
    with open(jsonl_path, "r", encoding="utf-8") as f:
        for line in f:
            ...
            video_name = item["vid"]
            video_path = os.path.join("TACoS/videos", f"{video_name}-cam-002.mp4")
...
if __name__ == "__main__":
    jsonl_path = "<path/to/tacos_train.jsonl>"
    video_dir = "<path/to/TACoS/videos>"
    save_path = "<path/to/output/FT_tacos_train.json>"
    ...
    convert_tacos_to_llamafactory_json(jsonl_path, video_dir, save_path, prompt_templates=prompt_templates)
