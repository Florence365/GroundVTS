...
def compute_hl_hit1_multiple(qid2preds, qid2gt_scores_binary):
    hit_scores_list = []
    
    for qid, pred_data in qid2preds.items():
        pred_scores = pred_data['pred_saliency_scores']
        gt_scores_binary = qid2gt_scores_binary[qid]
        
        max_score = np.max(pred_scores)

        if max_score == 0:
            hit_scores_list.append(0)
            continue
        
        max_indices = np.where(pred_scores == max_score)[0]
        
        # Check if at least one top-scored clip is a hit
        hit = 0
        for idx in max_indices:
            if idx < len(gt_scores_binary) and np.any(gt_scores_binary[idx] > 0):
                hit = 1
                break
        
        hit_scores_list.append(hit)
    
    hit_at_one = float(f'{100 * np.mean(hit_scores_list):.2f}')
    return hit_at_one
...
def eval_highlight(preds, annos):
    ...
    for gt_saliency_score_min, score_name in zip(gt_saliency_score_min_list, saliency_score_names):
        ...
        hit_at_one = compute_hl_hit1_multiple(qid2preds, qid2gt_scores_binary)
...
def parse_args():
    ...
    parser.add_argument('--pred_name', default='output_qvhighlights_valid_2.0')
    parser.add_argument('--anno_path', default='<path/to/qvhighlights_annotations>')
...
if __name__ == '__main__':
    ...
    pred_file = nncore.join(args.pred_path, f"{args.pred_name}.jsonl")
    nncore.log(f'Loading predictions from {pred_file}')
    preds = nncore.load(pred_file)
